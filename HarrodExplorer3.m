...
% Find the difference in selected switch points
differenceLow = switchPoints4(1) - switchPoints1(1);
...
[nSwitchPoints4, switchPoints4] = get_switch_points( ...
    fGamma, gGamma, fDelta, gDelta, 0, min( rMaxGamma, rMaxDelta ) );
% Find the difference
differenceHigh = switchPoints4(1) - switchPoints1(1);;
...
if ( differenceLow*differenceHigh < 0 )
  ...
  while ( abs( differenceHigh - differenceLow) > epsilon )
     ...
     [nSwitchPoints4, switchPoints4] = get_switch_points( ...
          fGamma, gGamma, fDelta, gDelta, 0, min( rMaxGamma, rMaxDelta ) );
     % Find the difference
     differenceNew = switchPoints4(1) - switchPoints1(1);;
