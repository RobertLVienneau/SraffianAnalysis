...
% -----------------
% Find example with switch point for Alpha and Beta on the wage axis..
phiInitial1 = 1;
phiInitial2 = 0.4;
...
[nSwitchPoints1, switchPoints1] = get_switch_points( ...
    fAlpha, gAlpha, fBeta, gBeta, -2, min( rMaxAlpha, rMaxBeta ) );
% Find the difference with -1
differenceLow = -1 - switchPoints1(1);;
...
[nSwitchPoints1, switchPoints1] = get_switch_points( ...
    fAlpha, gAlpha, fBeta, gBeta, -2, min( rMaxAlpha, rMaxBeta ) );
% Find the difference with -1
differenceHigh = -1 - switchPoints1(1);;
...
if ( differenceLow*differenceHigh < 0 )
  ...
  while ( abs( differenceHigh - differenceLow) > epsilon )
     ...
     [nSwitchPoints1, switchPoints1] = get_switch_points( ...
         fAlpha, gAlpha, fBeta, gBeta, -2, min( rMaxAlpha, rMaxBeta ) );
     % Find the difference with -1
     differenceNew = -1 - switchPoints1(1);
